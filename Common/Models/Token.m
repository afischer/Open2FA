...
static NSString* getOTP(CCHmacAlgorithm algo, uint8_t digits, NSData* key, uint64_t counter) {
...
//    self.digits = [query objectForKey:@"digits"];
    ...
    if (self.issuer == nil) {
        self.issuer = [query objectForKey:@"query"]; // THROW WARNING IF NOT EQUAL TO ISSUER ALREADY
    }
...
- (id)initWithMethod:(NSString *)method
    ...
    NSString *uri = [NSString stringWithFormat:@"otpauth://%@/%@:%@?&secret=%@",
                     method, [issuer percentEncoded], [account percentEncoded], secret];
    NSLog(@"THE UR I IS %@", uri);
    return [self initWithURI:[NSURL URLWithString:uri]];
}

- (NSString*)uid { // FIX THIS
    NSString *uidStr = [NSString stringWithFormat:@"%@:%@", self.issuer, self.method];
    return [uidStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
}

- (NSString *)tokenURI {
    
    NSString *uri = [NSString stringWithFormat:@"otpauth://%@/%@:%@?algorithm=%s&digits=%lu&secret=%@&issuer=%@&period=%u",
                     self.method, self.issuer, self.account, unparseAlgo(self.algorithm), (unsigned long)self.digits, [secret base32String], self.issuer, self.period];
    NSLog(@"GETTING URI %@", uri);
    return [uri stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
}
...
- (TokenCode *)code {
    ...
    if ([self.method isEqualToString:@"hotp"]) {
        // TODO: All algorithms
        NSString* code = getOTP(self.algorithm, self.digits, secret, self.counter++);
        return [[TokenCode alloc] initWithCode:code startTime:now endTime:now + self.period];
    }

    TokenCode* next = [[TokenCode alloc]
                       initWithCode:getOTP(self.algorithm, self.digits, secret, now / self.period + 1)
    ...
    return [[TokenCode alloc] initWithCode:getOTP(self.algorithm, self.digits, secret, now / self.period)
