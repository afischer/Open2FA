...
- (void)handleBackgroundTasks:
  ...
  for (WKRefreshBackgroundTask *task in backgroundTasks) {
    ...
    if ([task isKindOfClass:[WKApplicationRefreshBackgroundTask class]]) {
      ...
      // Snapshot tasks have a unique completion call, make sure to set your
      // expiration date
      WKSnapshotRefreshBackgroundTask *snapshotTask =
          (WKSnapshotRefreshBackgroundTask *)task;
      [snapshotTask
          setTaskCompletedWithDefaultStateRestored:YES
                       estimatedSnapshotExpiration:[NSDate distantFuture]
                                          userInfo:nil];
    } else if ([task isKindOfClass:
                         [WKWatchConnectivityRefreshBackgroundTask class]]) {
      // Be sure to complete the background task once you’re done.
